...
Improvements over demo.py:
  ...
  3. No periodic/timed auto-refresh. The list is built once at startup and then
     only rebuilt when:
        - the user clicks the "刷新 / Refresh" button, or
        - the main window genuinely (re)gains focus after having lost it.
"""
...
def ocr_image(img, win_id="", cache=None):
    """对截图做 OCR（tesseract CLI），返回识别出的文本。

    返回值约定：
      - 成功 -> 识别文本（可能为空字符串，表示该图没有可识别文字）
      - 失败 -> ""（调用方据此保留旧 OCR 文本）
      - 跳过（图像自上次 OCR 未变化）-> None（调用方据此不更新）

    NOTE: 这是阻塞调用 (blocking)，在专用的小型 OCR 线程池中并发执行。
    已应用的提速手段：
      F3 先把长边缩到 OCR_DOWNSCALE_LONG_EDGE 再识别（像素更少 -> 更快）；
      F4 用图像哈希跳过未变化的截图（避免 15s 后台刷新重复 OCR）；
      F1 OMP_THREAD_LIMIT 限制每个进程的线程数，杜绝多进程时的核数超额订阅；
      F7 --oem 1 --psm 6 仅用 LSTM 且按统一文本块切分；
      F6 timeout 限定单次最长耗时，并在超时时杀掉子进程。
    """
    ...
    if OCR_DOWNSCALE_LONG_EDGE and long_edge > OCR_DOWNSCALE_LONG_EDGE:
        work = img.copy()
        work.thumbnail((OCR_DOWNSCALE_LONG_EDGE, OCR_DOWNSCALE_LONG_EDGE), Image.LANCZOS)
    ...
    # F4: 若该窗口的图像与上次成功 OCR 的图像相同，则跳过。
    digest = None
    if cache is not None:
        digest = hashlib.blake2b(png_bytes, digest_size=16).digest()
        if cache.get(win_id) == digest:
            log.info("  OCR %s skipped (image unchanged since last OCR).", win_id)
            return None

    cmd = ['tesseract', 'stdin', 'stdout', '-l', OCR_LANG,
           '--oem', OCR_OEM, '--psm', OCR_PSM]               # F7
    env = {**os.environ, 'OMP_THREAD_LIMIT': str(OCR_OMP_THREADS)}  # F1
    try:
        proc = subprocess.run(cmd, input=png_bytes, capture_output=True,
                              env=env, timeout=OCR_TIMEOUT_S)        # F6
    except FileNotFoundError:
        log.warning("  OCR unavailable: 'tesseract' not found at OCR time.")
        return ""
    except subprocess.TimeoutExpired:
        elapsed = (time.perf_counter() - t0) * 1000
        log.warning("  OCR TIMEOUT for %s after %.0f ms (limit %ds); child killed.",
                    win_id, elapsed, OCR_TIMEOUT_S)
        return ""
    ...
    # 成功：记录哈希（即使文本为空也记，避免重复 OCR 同一张图）。
...
async def capture_stream(windows, executor, on_result, on_ocr=None,
                         ocr_executor=None, ocr_cache=None):
    ...
    截图成功后，再异步地对该图做 OCR，完成后回调 on_ocr(win_id, text)，把识别
    文本纳入搜索。OCR 跑在独立的小型线程池 ocr_executor 上（F2），与截图分离并
    限制并发，避免拖慢截图、也避免 CPU 超额订阅。ocr_image 返回 None 表示该图
    未变化被跳过（F4），此时不回调。
    """
    loop = asyncio.get_event_loop()
    log.info("Scheduling %d capture task(s) onto the executor ...", len(windows))

    ocr_exec = ocr_executor or executor
    ...
    async def one(win_id, title):
        ...
        if img is not None and on_ocr is not None and OCR_ENABLED and _OCR_AVAILABLE:
            text = await loop.run_in_executor(ocr_exec, ocr_image, img, win_id, ocr_cache)
            if text is not None:   # None = 图像未变化被跳过
                on_ocr(win_id, text)
...
# 全局字体（放大）。FONT_FAMILY 留空字符串则沿用系统默认字族。
FONT_FAMILY = ""              # 例如 "DejaVu Sans"、"Noto Sans CJK SC"；"" = 系统默认
...
# ---------------------------- 主 GUI 应用 / Main GUI ----------------------------
# 悬停放大预览的可调参数
HOVER_DELAY_S = 0.5            # 鼠标需在缩略图上静止多少秒后才弹出放大预览
...
REFRESH_INTERVAL_S = 15      # 后台自动刷新间隔（秒，仅在窗口未聚焦时运行）

# ---- OCR 配置 ----
OCR_ENABLED = True           # 截图更新后是否对其做 OCR，并把识别文本纳入搜索
OCR_LANG = "eng"     # tesseract 语言（需安装 tesseract-ocr-chi-sim / -eng 语言包）

# ---- OCR 提速参数（quick wins）----
OCR_MAX_WORKERS = 2          # F2: 专用 OCR 线程数（与截图线程池分离，限制并发，避免 CPU 超额订阅）
OCR_OMP_THREADS = 1          # F1: 每个 tesseract 进程的 OpenMP 线程数；=1 最稳，杜绝核数超额订阅
OCR_DOWNSCALE_LONG_EDGE = 1440  # F3: OCR 前把图像长边缩到该像素（<=0 表示不缩放）
OCR_TIMEOUT_S = 20           # F6: 单次 tesseract 超时（秒）；超时则放弃该窗口本轮 OCR
OCR_PSM = "6"                # F7: page segmentation mode（6 = 统一文本块）
OCR_OEM = "1"                # F7: OCR engine mode（1 = 仅 LSTM）
...
class WindowPreviewApp(tk.Tk):
    def __init__(self):
        ...
        # F2: 专用 OCR 线程池（与截图分离，限制并发，避免 CPU 超额订阅）
        self.ocr_executor = ThreadPoolExecutor(
            max_workers=max(1, OCR_MAX_WORKERS),
            thread_name_prefix="ocr",
        )
        # F4: 每个窗口上次成功 OCR 的图像哈希，用于跳过未变化的截图
        self._ocr_hash = {}

        # 状态标志
        self._is_refreshing = False   # 防止并发刷新
        self._has_focus = True        # 跟踪焦点状态
        self._auto_after = None       # 后台周期刷新的 after id（仅未聚焦时运行）
    ...
    def _show_preview(self, item, px, py):
        ...
        # 顶部标题栏（完整窗口标题，置于截图之上）
        title_lbl = tk.Label(
            tip, text=item['title'], anchor='w', justify='left',
    ...
    def on_deactivate(self, event):
        """整个应用窗口变为非活动（用户切到别的应用）-> 视为失焦，启动后台刷新。

        子控件之间移动焦点（例如点击搜索框）不会触发 <Deactivate>，
        因此不会被误判为失焦。
        """
        ...
        log.info("App deactivated (lost focus) -> enabling background auto-refresh.")
        self._start_auto()   # 未聚焦时恢复周期刷新

    def on_activate(self, event):
        """整个应用窗口变为活动 -> 视为聚焦：停止后台周期刷新，画面保持静止（不刷新）。"""
        if event.widget is not self:
            return
        if self._has_focus:
            return  # 已经处于活动状态
        self._has_focus = True
        self._stop_auto()   # 聚焦时取消周期刷新；聚焦期间不刷新
        log.info("App activated (gained focus) -> auto-refresh paused (display stays still).")

    # ------------------------ 后台周期刷新 / periodic refresh ------------------------
    def _start_auto(self):
        """在窗口未聚焦时启动周期刷新（若尚未启动）。"""
        if self._auto_after is None:
            log.info("Background auto-refresh enabled (every %ss while unfocused).",
                     REFRESH_INTERVAL_S)
            self._auto_after = self.after(int(REFRESH_INTERVAL_S * 1000), self._auto_tick)

    def _stop_auto(self):
        """停止周期刷新。"""
        if self._auto_after is not None:
            log.info("Background auto-refresh disabled.")
            self.after_cancel(self._auto_after)
            self._auto_after = None

    def _auto_tick(self):
        """周期刷新的一拍：仅在仍未聚焦时刷新并重新排期。"""
        self._auto_after = None
        if self._has_focus:
            return
        log.info("Background auto-refresh tick.")
        self.refresh()
        if not self._has_focus:
            self._auto_after = self.after(int(REFRESH_INTERVAL_S * 1000), self._auto_tick)
    ...
    def refresh(self, force=False):
        """增量刷新：保留可访问窗口的磁贴，仅移除消失的、新增新出现的，
        截图阶段并发执行，逐张回到主线程更新（不冻结、不清空整列表）。

        默认仅在窗口“未聚焦”时执行——窗口聚焦时保持画面静止，方便用户点击。
        force=True 用于显式手动刷新（刷新按钮 / 首次加载），无视聚焦状态。
        """
        if self._has_focus and not force:
            log.info("Window is focused -> skipping refresh (only refresh when unfocused).")
            return
        ...
        # 调和列表：移除消失的、新增新出现的、按顺序重排并重新布局。
        # 注意：不清空整列表，已有磁贴继续可见、可点击、可滚动。
        ...
        def worker():
            ...
            try:
                asyncio.run(capture_stream(windows, self.executor, on_result, on_ocr,
                                           ocr_executor=self.ocr_executor,
                                           ocr_cache=self._ocr_hash))
            except Exception as exc:
    ...
    def _reconcile(self, windows):
        ...
        for item in list(self.window_items):
            if item['win_id'] not in current_set:
                ...
                self._ocr_hash.pop(item['win_id'], None)
                removed += 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        app.ocr_executor.shutdown(wait=False)
