...
Power-efficient refresh (this variant):
  The first refresh (startup) and any manual "刷新 / Refresh" click do a full
  sweep — every window is captured at once, so the grid fills immediately.
  After that, each periodic tick (every REFRESH_INTERVAL_S seconds) captures
  only:
...
class WindowPreviewApp(tk.Tk):
    ...
    def refresh(self, force=False):
        ...
        - force=True（首次加载 / 手动「刷新」按钮）：全量刷新——截图所有窗口。
        - force=False（周期一拍）：省电刷新——只更新「当前聚焦窗口 + 至多 Y 个
          后台窗口」，后台窗口取自一个打乱的轮转队列（耗尽后重新收集并打乱）。

        无论哪种模式，都先用 wmctrl 取窗口列表并调和磁贴（增/删/重排，很轻量），
        仅「截图」这一步按模式决定是全量还是一小批。
        """
        ...
        log.info("=== Refresh START (%s) ===", "FULL" if force else "partial")
        t0 = time.perf_counter()
        ...
        if force:
            # 全量刷新：截图所有窗口（首次加载 / 手动刷新）。
            capture_list = list(windows)
            log.info("Full refresh: capturing all %d window(s).", len(capture_list))
        else:
            # 省电一拍：聚焦窗口 + 一批轮转的后台窗口。
            # 2) 确定当前聚焦窗口（每拍都更新它）。
